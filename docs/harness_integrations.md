...
прочие секции — заголовок + первые два абзаца. Итог усечён до 6000 символов
(≈1500 токенов при оценке 4 символа/токен) с пометкой об усечении. Смысл:
агент реализует без доступа к исходным документам и без архитектурных
изобретений.
...
timeout_secs = 1800               # дефолт 1800; по таймауту процесс убивается
...
- Дефолтный `CONSTRAINTS.yaml` пакета: `no-unwrap-in-src` (warn),
  `no-dbg-macro` (error), `readme-exists` (warn), `cargo-check-passes`
  (error, таймаут 120 с) — схема правил в `docs/control.md`.
