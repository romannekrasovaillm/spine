...
Агенты забывают, файлы — нет. Передача архитектуры в реализацию — через пакет файлов `.arch-handoff/`, а не через устную постановку (инструмент `handoff_create`, CLI `arch handoff`).
...
1. `arch harness-run <harness> --repo <path>` — прогон (таймаут из конфига).
2. `arch control check <repo>` — fitness functions по CONSTRAINTS.yaml (гейт A4; exit 1 при FAIL — в CI).
3. `arch rubric run handoff_quality <итог.md>` — оценка результата рубрикой.
